...
# router 级"是否云管角色"由 main.py 的 _cloud("bills") 完成。
# 数据范围:bills 表无 cloud_account_id 但有 provider — 按 provider 过滤。
router = APIRouter()


async def _load_bill_in_scope(
    db: AsyncSession, bill_id: int, principal: Principal
) -> MonthlyBill:
    """读取 bill + 校验 provider 在用户范围内。"""
    ...
    if not has_full_access(principal):
        ...
        if bill.provider not in scope_providers:
            raise HTTPException(403, f"Provider '{bill.provider}' out of scope")
    return bill
...
async def list_bills(
    ...
    # 数据范围:cloud_<provider> 只看本云的账单
    if not has_full_access(principal):
        scope_providers = extract_providers_from_roles(principal.roles)
        if not scope_providers:
            return []
        stmt = stmt.where(MonthlyBill.provider.in_(scope_providers))
    if month:
...
async def generate(
    ...
    # 全量生成 — 仅 admin/ops(批量动作不细分 provider)
    if not has_full_access(principal):
        raise HTTPException(403, "Only cloud_admin/cloud_ops can run bill generate.")
...
async def get_bill(
    ...
    return await _load_bill_in_scope(db, bill_id, principal)


@router.put("/{bill_id}/adjust", response_model=MonthlyBillRead)
async def adjust_bill(
    bill_id: int,
    body: MonthlyBillAdjust,
    db: AsyncSession = Depends(get_db),
    principal: Principal = Depends(get_current_principal),
):
    bill = await _load_bill_in_scope(db, bill_id, principal)
...
async def regenerate(
    ...
    if not has_full_access(principal):
        raise HTTPException(403, "Only cloud_admin/cloud_ops can run bill regenerate.")
