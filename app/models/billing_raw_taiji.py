...
业务流：
    taiji → POST /api/metering/taiji/ingest → 去重入这张表 → 按涉及日期重算 billing_summary + token_usage
...
class BillingRawTaiji(Base):
    __tablename__ = "billing_raw_taiji"
    __table_args__ = (
        PrimaryKeyConstraint("data_source_id", "id", name="pk_billing_raw_taiji"),
        Index("ix_billing_raw_taiji_ds_date", "data_source_id", "date"),
        Index("ix_billing_raw_taiji_ds_ingested", "data_source_id", "ingested_at"),
    )
